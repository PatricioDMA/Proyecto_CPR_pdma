...
# Etiquetas para los dos casos comparados
etiquetas = [
    "Sin error",
    "Peor caso"
]
...
if len(csv_files) != 2:
    print("AVISO: Se esperaban exactamente 2 archivos 'results_*.csv'. Encontrados:", len(csv_files))
    exit()
...
# Decoración
plt.title('Comparación de RMSE frente al tiempo (2 casos)')
plt.xlabel('Tiempo (s)')
plt.ylabel('RMSE')
...
# Guardar
output_path = os.path.join(current_dir, 'grafica_comparativa_rmse_2.png')
