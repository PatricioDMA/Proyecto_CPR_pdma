...
class PurePursuitNode(Node):
    def __init__(self):
        ...
        self.tracking_errors = []
         ...
         # Declarar parámetros de configuración y obtener sus valores
        self.declare_parameter('use_noisy_odom', False)
        self.declare_parameter('use_drive_raw', False)
        self.declare_parameter('use_latency', False)

        use_noisy_odom = self.get_parameter('use_noisy_odom').get_parameter_value().bool_value
        use_drive_raw = self.get_parameter('use_drive_raw').get_parameter_value().bool_value
        use_latency = self.get_parameter('use_latency').get_parameter_value().bool_value

        # Tópico de odometría (ruido o no)
        odom_topic = '/ego_racecar/odom_noisy' if use_noisy_odom else '/ego_racecar/odom'
        self.create_subscription(Odometry, odom_topic, self.odom_callback, 10)
        self.get_logger().info(f"Suscrito a: {odom_topic}")

        # Tópico de publicación (raw o directo)
        drive_topic = '/drive_raw' if use_drive_raw else '/drive'
        self.drive_pub = self.create_publisher(AckermannDriveStamped, drive_topic, 10)
        self.get_logger().info(f"Publicando en: {drive_topic}")

        # Latencia en el control (si se activa)
        if use_latency:
            self.get_logger().info("Modo LATENCIA: 5 Hz")
            self.timer = self.create_timer(0.2, self.control_loop)  # 5 Hz
        else:
            self.get_logger().info("Modo NORMAL: 20 Hz")
