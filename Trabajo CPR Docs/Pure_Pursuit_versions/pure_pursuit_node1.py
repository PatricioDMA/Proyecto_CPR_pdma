...
class PurePursuitNode(Node):
    ...
    def lidar_callback(self, msg):
        ...
        # Convertir ángulos a índices del array de ranges
        start_angle = math.radians(self.lidar_front_angles[0])
        end_angle = math.radians(self.lidar_front_angles[1])
        ...
        # Comprobar si algún valor en ese rango está por debajo del umbral
        for i in range(start_index, end_index + 1):
            distance = ranges[i]
            if msg.range_min < distance < self.min_obstacle_distance:
    ...
    def control_loop(self):
        ...
        if self.obstacle_detected:
            self.get_logger().warn("¡Obstáculo frontal detectado! Deteniendo vehículo.")
